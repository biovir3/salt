...
def saltadm_account(sminion, saltops_group):
    try:
        ...
        assert sminion.functions.shadow.set_password(
            USERB,
            USERB_PWD if salt.utils.platform.is_darwin() else HASHED_USERB_PWD
        )
...
class TestUserAuth(object):
    ...
    def test_pam_auth_valid_user(self, salt_cli, saltdev_account):
        ...
        ret = salt_cli.run('-a', 'pam', '--username', USERA, '--password', USERA_PWD, 'test.ping', minion_tgt='minion')
    ...
    def test_pam_auth_invalid_user(self, salt_cli, saltdev_account):
        ...
        '''
        ret = salt_cli.run('-a', 'pam', '--username', 'nouser', '--password', '1234', 'test.ping', minion_tgt='minion')
...
class TestGroupAuth(object):
    ...
    def test_pam_auth_valid_group(self, salt_cli, saltadm_account):
        ...
        ret = salt_cli.run('-a', 'pam', '--username', USERB, '--password', USERB_PWD, 'test.ping', minion_tgt='minion')
        assert ret.exitcode == 0
        assert ret.json is True
